...
function! s:update_checkbox_percentage()
  ...
  let bullet = matchstr(line, '^\s*\zs[-*+]\ze\s\+\[')
  if empty(bullet)
    let bullet = '*'
  endif
    ...
    if parent_indent < current_indent && getline(parent_line) !~ '^\s*$'
      ...
      if !empty(match)
        ...
        if child_percentage >= 0
          ...
          let new_content = substitute(line_content, '\[\(.\)\]', '[' . new_state . ']', '')
          
          " Remove existing percentage
          let new_content = substitute(new_content, '\s\+\d\+%\s*
...
function! VimwikiSetPercentage(percentage)
  ...
  " Remove existing percentage
  let new_line = substitute(new_line, '\s\+\d\+%\s*
...
augroup VimwikiPercentageSyntax
  autocmd!
  autocmd FileType vimwiki syntax match VimwikiPercentage '\d\+%' contained
